...
def set_group(message):
    """Команда /set_group - Установить один `group_id` для отправки"""
    ...
    try:
        ...
        bot.reply_to(message, f"✅ Группа установлена!\n\n📋 ID: `{group_id}`", parse_mode='Markdown')
        logger.info(f"Группа установлена: {group_id}")
    except ValueError:
        bot.reply_to(message, "❌ Неправильный ID группы!", parse_mode='Markdown')


@bot.message_handler(commands=['get_group'])
def get_group(message):
    """Команда /get_group - Показать установленный `group_id`"""
    group_id = messages_storage.get('group_id')
    if group_id:
        bot.reply_to(message, f"📋 Текущий group_id: `{group_id}`", parse_mode='Markdown')
    else:
        bot.reply_to(message, "❌ Group_id не установлен. Используйте `/set_group -100...`", parse_mode='Markdown')
...
def send_message_cmd(message):
    ...
    if not group_id:
        bot.reply_to(message,
            "❌ Group_id не установлен!\n\nИспользуйте: `/set_group -1001234567890`",
            parse_mode='Markdown')
        return

    message_text = messages_storage.get('send_message_text', '')
    try:
        bot.send_message(chat_id=group_id, text=f"📤 {message_text}", parse_mode='Markdown')
        ...
        bot.reply_to(message, f"❌ Ошибка при отправке в группу `{group_id}`.")
...
def scheduled_sender():
    ...
    while not stop_scheduler:
        try:
            # Получаем один group_id (как раньше)
            group_id = messages_storage.get('group_id')
                ...
                if day_schedule:
                    for time_slot, schedule_text in day_schedule.items():
                        ...
                        if time_slot == current_time and send_key not in last_sent_times:
                            ...
                            try:
                                bot.send_message(chat_id=group_id, text=f"🤖 *{schedule_text}*", parse_mode='Markdown')
                                logger.info(f"✅ УСПЕШНО ОТПРАВЛЕНО (неделя) в {group_id}!")
                            except Exception as e:
                                logger.error(f"❌ Ошибка при отправке (неделя) в {group_id}: {e}")
                ...
                if daily_schedule:
                    for time_slot, schedule_text in daily_schedule.items():
                        ...
                        if time_slot == current_time and send_key not in last_sent_times:
                            ...
                            try:
                                bot.send_message(chat_id=group_id, text=f"🤖 *{schedule_text}*", parse_mode='Markdown')
                                logger.info(f"✅ УСПЕШНО ОТПРАВЛЕНО (ежедневно) в {group_id}!")
                            except Exception as e:
                                logger.error(f"❌ Ошибка при отправке (ежедневно) в {group_id}: {e}")
                            last_sent_times.add(send_key)
                        elif time_slot != current_time and send_key in last_sent_times:
                            last_sent_times.discard(send_key)
                ...
                if one_off_today:
                    for time_slot, schedule_text in one_off_today.copy().items():
                        ...
                        if time_slot == current_time and send_key not in last_sent_times:
                            ...
                            try:
                                bot.send_message(chat_id=group_id, text=f"🤖 *{schedule_text}*", parse_mode='Markdown')
                                logger.info(f"✅ УСПЕШНО ОТПРАВЛЕНО (one-off) в {group_id}!")
                            except Exception as e:
                                logger.error(f"❌ Ошибка при отправке (one-off) в {group_id}: {e}")
                            last_sent_times.add(send_key)
